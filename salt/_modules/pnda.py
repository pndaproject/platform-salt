...
def get_hosts_for_role(role):
    ...
    domain = __salt__['pillar.get']('consul:domain')
    data_center = __salt__['pillar.get']('consul:data_center')
    node = __salt__['pillar.get']('consul:node')
    domain_name = '.%s.%s.%s' % (node, data_center, domain)
    result = ['%s%s' % (host_name, domain_name) for host_name in result]
    return result if len(result) > 0 else None
